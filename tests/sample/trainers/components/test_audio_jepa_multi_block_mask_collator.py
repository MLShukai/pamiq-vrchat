...
from sample.trainers.components.audio_jepa_multi_block_mask_collator import (
    AudioJEPAMultiBlockMaskCollator,
)
...
class TestAudioJEPAMultiBlockMaskCollator:
    ...
    def test_sample_mask(
        ...
        # define AudioJEPAMultiBlockMaskCollator
        collator = AudioJEPAMultiBlockMaskCollator(
    ...
    def test_bool_i_jepa_mask_collator(
        ...
        audios = [
            (torch.randn([n_channels, input_sample_size]),) for _ in range(batch_size)
        ]
        ...
        assert (
            collated_audios.size(2) == input_sample_size
