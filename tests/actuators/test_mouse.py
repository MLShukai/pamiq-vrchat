...
from pamiq_vrchat.actuators.mouse import MOUSE_BUTTONS, MouseAction, MouseActuator
...
class TestMouseActuator:
    ...
    def test_operate_buttons(self, mock_mouse_output):
        ...
        actuator = MouseActuator()
        action: MouseAction = {"button_press": {"left": True, "right": False}}

        actuator.operate(action)

        # Verify press and release were called appropriately
        mock_instance.press.assert_called_once_with("left")
        mock_instance.release.assert_called_once_with("right")
    ...
    def test_on_paused(self, mock_mouse_output):
        ...
        assert mock_instance.release.call_count == len(MOUSE_BUTTONS)
